// ER 急診站「佈告欄」假資料（待接 API）。
// 模擬後端回應結構：Success/Message + Data。Data 內含：
//   WardCode 病房代碼、QueryDate 查詢日期、
//   UnitBulletins 本單位（急診室）公告、HospBulletins 院方公告。
// 每筆公告欄位：BulletinId 編號、Category 分類、Title 標題、Content 內文、
//   Priority 優先度（重要/一般）、PostedBy 發布者、PostedAt 發布日期、IsActive 是否啟用。
// 註：實際 BulletinTab 已改由 textApi 取得，此檔為早期假資料樣本。
const BULLETIN_DATA = {
  Success: true,
  Message: "",
  Data: {
