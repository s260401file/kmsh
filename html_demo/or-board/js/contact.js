...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const res = await getContacts("OR", "2026-06-03");
  const { DutyContacts, CommonContacts } = res.Data;
  document.getElementById("duty-list").innerHTML = DutyContacts.map(c => `
    <tr><td>${c.DutyTitle}</td><td>${c.Name}</td><td class="ct-ext">${c.Extension}</td><td class="ct-mobile">${c.Mobile}</td><td class="ct-slot">${c.TimeSlot}</td></tr>`).join("");
  document.getElementById("common-list").innerHTML = CommonContacts.map(c => `
    <tr><td>${c.Name}</td><td class="ct-col-ext ct-ext">${c.Extension}</td></tr>`).join("");
});
