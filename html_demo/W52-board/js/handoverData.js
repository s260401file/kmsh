...
// §7.1.2 (11) 交班日誌摘要
//
// 註：規格僅列「交班日誌摘要」；DB schema 尚未訂定專用表，
//     此處依實務 SBAR / 重點交班結構設計欄位，正式版可對應
//     未來新增的 ShiftHandover / HandoverLog 表
//
// 欄位（PascalCase = C# Model）：
//   HandoverInfo ： FromShift, FromShiftTime, ToShift, ToShiftTime,
//                   HandoverTime(HH:mm), FromNurses[], ToNurses[]
//   Patients[]   ： HandoverId, BedNo, PatientName, Gender, Age,
//                   Diagnosis, Priority("高"|"中"|"一般"),
//                   Items[] (Category, Content)
//
// 正式上線時替換 getHandover() 內部為 fetch() 呼叫即可
// ──────────────────────────────────────────────────────────────

const _MOCK_HANDOVER = {
  "Success": true,
  "Message": "",
  "Data": {
    "WardCode": "W52",
    ...
    "HandoverInfo": {
      "FromShift":     "白班",
      "FromShiftTime": "08:00–16:00",
      "ToShift":       "小夜",
      "ToShiftTime":   "16:00–24:00",
      "HandoverTime":  "16:00",
      "FromNurses":    ["陳○梅", "蔡○柔", "王○惠", "黃○萍"],
      "ToNurses":      ["鄭○雲", "林○靜"]
    },
    ...
    "Patients": [
      ...
      },
      {
        ...
        "Gender": "F",
        "Age": 62,
        "Diagnosis": "Type 2 DM 合併足部蜂窩性組織炎",
        "Priority": "中",
        "Items": [
...
// ── API 模擬函式 ──────────────────────────────────────────────
// React 遷移：useEffect(() => { getHandover(wardCode, date).then(setData) }, [wardCode, date])
// TODO 正式上線：return fetch(`/api/wards/${wardCode}/handover?date=${date}`).then(r => r.json())
async function getHandover(wardCode, date) {
  return Promise.resolve(_MOCK_HANDOVER);
}
