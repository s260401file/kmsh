...
## 現況（一句話）
院方原無 AD，依 2026-07-02 會議決議，於 **101 主機**（即本應用主機）架 **AD LDS**（Windows 內建輕量 LDAP，非網域控制站），供白板管理後台「**每人專屬帳號＋登入／登出留存（可勾稽）**」。白板透過 **LDAP bind** 驗證密碼；單位／角色權限仍用本地 `StaffUnitRole`。已上線、58 位在職人員帳號可登入。**帳號管理亦已連動**（2026-07-10）：後台建帳號／更名／停用／重設密碼、使用者自助改密皆寫入 AD LDS，不再靠手動 PS（詳 [[帳號連動AD-規劃]]）。

## 文件
- [[AD-LDS-安裝記錄]] — 安裝決策、實例設定、踩過的雷、維運（改密／新增／停用／LDAPS／備份）。
- [[帳號連動AD-規劃]] — **已實作（2026-07-10）**：後台建帳號／更名／停用／管理員重設＋使用者自助改密，皆連動 AD LDS。實作於 `Services/LdapAdminService.cs`（app pool 改用專用服務帳號 `.\kmshldapsvc` 寫入）。原「規劃」背景保留於該文件。
