...
> ⚠⚠ **勿被程式碼誤導**：`API/` 的 `VghksApiService` / `AmdrCase.patflag` 雖已寫出 dnr/fall/iso/npo/主責護理師/會診 等欄位，那是**對接高榮規格的「預留殼」，民生大部份無實際資料**。本頁「A. HIS 已可提供」指的是**字典裡表結構存在**，非「民生現在拿得到」。判斷可用性以本頁與 [[資料項對照表]] 為準。詳 [[_院方API總覽]] 的預留說明。
> ⚠⚠ **更進一步：有欄位 ≠ 有值**（院方 2026-06-22 確認）。本頁「A. 已可提供」即使欄位存在，**實際多為空值**（HPBASIC 僅姓名/身分證/病歷號/性別/生日有值；血型/身高體重/DNR/保密/安寧、急診 DNR 類皆空）。空值欄位**視同沒有→需自建**。逐欄實況與設計見 [[欄位資料實況]]。
