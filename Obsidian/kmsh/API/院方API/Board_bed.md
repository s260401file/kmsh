...
| 負責醫師 | `"陳蕙君"` | ★2026-07 起院方已帶（直接顯示，後台免自建）| HDOCTOR |
| 轉入日期 | ISO | 入院/轉入日；前端算住院天數 | — |
| 科別 | `"GS"` | 科別（院方已帶）| HSECTION |
| 診斷 | 文字 | 診斷（院方已帶）| HDIAGNOS |
| 動態 | `"A"` | ★2026-07 起新增；病患動向**代碼**（見下表），W52/ICU 病床卡彈窗「動向狀態」顯示 | HPATSTAT |
| 抗生素 | 藥名 | ★2026-07 起把**全部用藥** join 進來（欄名為「抗生素」，實為全用藥；同病人每筆一列）→ 後端去重彙整，餵 ICU 抗生素頁 | — |

## 「動態」代碼對照（2026-07-27 確認，與 [[Board_ER]] 病患動向不同表）
| 代碼 | 中文 | | 代碼 | 中文 |
|---|---|---|---|---|
| A | 住院中 | | I | 通知出院 |
| D | 已出院 | | M | 允許出院 |
| E | 病故 | | T | 轉院 |
> 目前住院在床多為 `A`。前端 `MOVEMENT_LABEL` 對照顯示；純顯示、不做統計/篩選。
