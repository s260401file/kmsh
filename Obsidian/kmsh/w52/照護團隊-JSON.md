...
## ⚠ 版面更新（2026-07，已上線）— 照服員/聯絡電話併入值班表
> 病室動態 [[W52病室動態-JSON與組裝]] 內「值班表」面板第 3 段已上線 **照服員＋聯絡電話**兩區（`TeamTab` 分頁保留完整照護團隊）：
> - **照服員** ← 後台「W52 管理→顯示照服員」`getUnitCareAides('W52')` → `[{ aideId, name, contact }]`（引用全院共用照服員主檔 `CareAide`，依所設順序）。
> - **聯絡電話** ← 後台「顯示聯絡電話」`getPhone('W52')` → `[{ id, title, name, extension }]`。
> 「照服員／聯絡電話」為各站共用引用模式（W52/ICU 同款；ER 另有照服員標籤列於三班醫護面板）。
